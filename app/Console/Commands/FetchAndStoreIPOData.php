...
class FetchAndStoreIPOData extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($statuses as $status) {
            $apiUrl = 'https://api.ipoalerts.in/ipos?status=' . $status;
            ...
            if ($response->failed()) {
                $this->error("Failed to fetch data for status: $status");
                continue;
            }
            $data = $response->json();

            if (!isset($data['ipos']) || !is_array($data['ipos'])) {
                $this->error("Invalid structure for status: $status");
                continue;
            }
            ...
            foreach ($data['ipos'] as $ipoData) {
                ...
                IPO::updateOrCreate(
                    ...
                    [
                        ...
                        'nse_info_url' => array_key_exists('nseInfoUrl', $ipoData) ? $ipoData['nseInfoUrl'] : null,
                        'schedule' => array_key_exists($ipoData['schedule'], $ipoData) ? json_encode($ipoData['schedule']) : null,
                    ]
                );
